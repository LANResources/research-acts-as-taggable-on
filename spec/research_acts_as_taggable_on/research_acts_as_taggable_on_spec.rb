...
describe 'Acts As Taggable On' do
  ...
  describe 'Taggable Method Generation To Preserve Order' do
    before(:each) do
      ...
      TaggableModel.research_acts_as_ordered_taggable_on(:ordered_tags)
  ...
  describe 'Taggable Method Generation' do
    before(:each) do
      TaggableModel.tag_types = []
      TaggableModel.research_acts_as_taggable_on(:tags, :languages, :skills, :needs, :offerings)
      @taggable = TaggableModel.new(name: 'Bob Jones')
    end
  ...
  describe 'Matching Contexts' do
    ...
    it 'should ensure joins to multiple taggings maintain their contexts when aliasing' do
      ...
      offer_alias = TaggableModel.connection.quote_table_name(ResearchActsAsTaggableOn.taggings_table)
  ...
  describe 'Tagging Contexts' do
    it 'should eliminate duplicate tagging contexts ' do
      TaggableModel.research_acts_as_taggable_on(:skills, :skills)
      expect(TaggableModel.tag_types.freq[:skills]).to eq(1)
    end

    it 'should not contain embedded/nested arrays' do
      TaggableModel.research_acts_as_taggable_on([:array], [:array])
    ...
    it 'should not raise an error when passed nil' do
      expect(-> {
        TaggableModel.research_acts_as_taggable_on
      }).to_not raise_error
    end

    it 'should not raise an error when passed [nil]' do
      expect(-> {
        TaggableModel.research_acts_as_taggable_on([nil])
      }).to_not raise_error
    end
  ...
  describe '@@remove_unused_tags' do
    before do
      ...
      @tagging = ResearchActsAsTaggableOn::Tagging.create(taggable: @taggable, tag: @tag, context: 'tags')
    end
    ...
    context 'if set to true' do
      ...
      it 'should remove unused tags after removing taggings' do
        @tagging.destroy
        expect(ResearchActsAsTaggableOn::Tag.find_by_name('awesome')).to be_nil
      end
    end

    context 'if set to false' do
      before do
        ResearchActsAsTaggableOn.remove_unused_tags = false
      end

      it 'should not remove unused tags after removing taggings' do
        @tagging.destroy
        expect(ResearchActsAsTaggableOn::Tag.find_by_name('awesome')).to eq(@tag)
      end
    end
  end
