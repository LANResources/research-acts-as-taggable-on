...
describe ResearchActsAsTaggableOn::TagsHelper do
  before(:each) do
  ...
  it 'should handle tags with zero counts (build for empty)' do
    ResearchActsAsTaggableOn::Tag.create(name: 'php')
    ResearchActsAsTaggableOn::Tag.create(name: 'java')
    ResearchActsAsTaggableOn::Tag.create(name: 'c++')

    tags = {}

    @helper.tag_cloud(ResearchActsAsTaggableOn::Tag.all, %w(sucky awesome)) do |tag, css_class|
      tags[tag.name] = css_class
    end
