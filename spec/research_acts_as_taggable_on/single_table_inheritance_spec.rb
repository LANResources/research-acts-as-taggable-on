...
describe 'Single Table Inheritance' do
  ...
  describe 'tag list' do
    ...
    it 'should have different tag_counts_on for inherited models' do
      ...
      expect(InheritingTaggableModel.tag_counts_on(:tags, order: "#{ResearchActsAsTaggableOn.tags_table}.id").map(&:name)).to eq(%w(bob kelso))
      expect(AlteredInheritingTaggableModel.tag_counts_on(:tags, order: "#{ResearchActsAsTaggableOn.tags_table}.id").map(&:name)).to eq(%w(fork spoon))
      expect(TaggableModel.tag_counts_on(:tags, order: "#{ResearchActsAsTaggableOn.tags_table}.id").map(&:name)).to eq(%w(bob kelso fork spoon))
    end
    ...
    it 'should have different tags_on for inherited models' do
      ...
      expect(InheritingTaggableModel.tags_on(:tags, order: "#{ResearchActsAsTaggableOn.tags_table}.id").map(&:name)).to eq(%w(bob kelso))
      expect(AlteredInheritingTaggableModel.tags_on(:tags, order: "#{ResearchActsAsTaggableOn.tags_table}.id").map(&:name)).to eq(%w(fork spoon))
      expect(TaggableModel.tags_on(:tags, order: "#{ResearchActsAsTaggableOn.tags_table}.id").map(&:name)).to eq(%w(bob kelso fork spoon))
    end
  ...
  describe 'a subclass of Tag' do
    ...
    it 'does not interfere with a normal Tag context on the same model' do
      ...
      company.save!
      expect(ResearchActsAsTaggableOn::Tag.where(name: 'cambridge', type: nil)).to_not be_empty
    end
