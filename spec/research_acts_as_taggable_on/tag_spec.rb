...
shared_examples_for 'without unique index' do
  ...
  append_after(:all) do
    ResearchActsAsTaggableOn::Tag.delete_all
    AddMissingUniqueIndices.up
  end
end

describe ResearchActsAsTaggableOn::Tag do
  before(:each) do
    @tag = ResearchActsAsTaggableOn::Tag.new
  ...
  describe 'named like any' do
    context 'case insensitive collation and unique index on tag name', if: using_case_insensitive_collation? do
      before(:each) do
        ResearchActsAsTaggableOn::Tag.create(name: 'Awesome')
        ResearchActsAsTaggableOn::Tag.create(name: 'epic')
      end

      it 'should find both tags' do
        expect(ResearchActsAsTaggableOn::Tag.named_like_any(%w(awesome epic)).count).to eq(2)
      end
    end
    ...
    context 'case insensitive collation without indexes or case sensitive collation with indexes' do
      ...
      it 'should find both tags' do
        expect(ResearchActsAsTaggableOn::Tag.named_like_any(%w(awesome epic)).count).to eq(3)
      end
    end
  end
  ...
  describe 'named any' do
    context 'with some special characters combinations', if: using_mysql? do
      it 'should not raise an invalid encoding exception' do
        expect{ResearchActsAsTaggableOn::Tag.named_any(["holä", "hol'ä"])}.not_to raise_error
      end
    end
  end
  ...
  describe 'for context' do
    ...
    it 'should return tags that have been used in the given context' do
      expect(ResearchActsAsTaggableOn::Tag.for_context('skills').pluck(:name)).to include('ruby')
    end

    it 'should not return tags that have been used in other contexts' do
      expect(ResearchActsAsTaggableOn::Tag.for_context('needs').pluck(:name)).to_not include('ruby')
    end
  end
  ...
  describe 'for tenant' do
    ...
    it 'should return tags for the tenant' do
      expect(ResearchActsAsTaggableOn::Tag.for_tenant('100').pluck(:name)).to include('ruby')
    end

    it 'should not return tags for other tenants' do
      expect(ResearchActsAsTaggableOn::Tag.for_tenant('200').pluck(:name)).to_not include('ruby')
    end
  end
  ...
  describe 'find or create by name' do
    ...
    it 'should find by name' do
      expect(ResearchActsAsTaggableOn::Tag.find_or_create_with_like_by_name('awesome')).to eq(@tag)
    end
    ...
    it 'should create by name' do
      expect(-> {
        ResearchActsAsTaggableOn::Tag.find_or_create_with_like_by_name('epic')
      }).to change(ResearchActsAsTaggableOn::Tag, :count).by(1)
    end
  end
  ...
  describe 'find or create by unicode name', unless: using_sqlite? do
    ...
    it 'should find by name' do
      expect(ResearchActsAsTaggableOn::Tag.find_or_create_with_like_by_name('привет')).to eq(@tag)
    end
    ...
    it 'should find by name accent insensitive', if: using_case_insensitive_collation? do
      @tag.name = 'inupiat'
      @tag.save
      expect(ResearchActsAsTaggableOn::Tag.find_or_create_with_like_by_name('Iñupiat')).to eq(@tag)
    end
  end
  ...
  describe 'find or create all by any name' do
    ...
    it 'should find by name' do
      expect(ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name('awesome')).to eq([@tag])
    end
    ...
    context 'case sensitive' do
      ...
      it 'should find by name case sensitive' do
        ...
        expect {
          ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name('AWESOME')
        }.to change(ResearchActsAsTaggableOn::Tag, :count).by(1)
      end
    end
    ...
    it 'should create by name' do
      expect {
        ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name('epic')
      }.to change(ResearchActsAsTaggableOn::Tag, :count).by(1)
    end
    ...
    context 'case sensitive' do
      ...
      it 'should find or create by name case sensitive' do
        ResearchActsAsTaggableOn.strict_case_match = true
        expect {
          expect(ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name('AWESOME', 'awesome').map(&:name)).to eq(%w(AWESOME awesome))
        }.to change(ResearchActsAsTaggableOn::Tag, :count).by(1)
      end
    end
    ...
    it 'should find or create by name' do
      expect {
        expect(ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name('awesome', 'epic').map(&:name)).to eq(%w(awesome epic))
      }.to change(ResearchActsAsTaggableOn::Tag, :count).by(1)
    end

    it 'should return an empty array if no tags are specified' do
      expect(ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name([])).to be_empty
    end
  end
  ...
  it 'should equal a tag with the same name' do
    @tag.name = 'awesome'
    new_tag = ResearchActsAsTaggableOn::Tag.new(name: 'awesome')
  ...
  it 'have named_scope named(something)' do
    @tag.name = 'cool'
    @tag.save!
    expect(ResearchActsAsTaggableOn::Tag.named('cool')).to include(@tag)
  end

  it 'have named_scope named_like(something)' do
    @tag.name = 'cool'
    @tag.save!
    @another_tag = ResearchActsAsTaggableOn::Tag.create!(name: 'coolip')
    expect(ResearchActsAsTaggableOn::Tag.named_like('cool')).to include(@tag, @another_tag)
  end
  ...
  describe 'escape wildcard symbols in like requests' do
    before(:each) do
      @tag.name = 'cool'
      @tag.save
      @another_tag = ResearchActsAsTaggableOn::Tag.create!(name: 'coo%')
      @another_tag2 = ResearchActsAsTaggableOn::Tag.create!(name: 'coolish')
    end

    it "return escaped result when '%' char present in tag" do
      expect(ResearchActsAsTaggableOn::Tag.named_like('coo%')).to_not include(@tag)
      expect(ResearchActsAsTaggableOn::Tag.named_like('coo%')).to include(@another_tag)
    end
  ...
  describe 'when using strict_case_match' do
    ...
    after do
      ResearchActsAsTaggableOn.strict_case_match = false
    end
    ...
    context 'case sensitive' do
      ...
      it 'should find by name case sensitively' do
        expect {
          ResearchActsAsTaggableOn::Tag.find_or_create_with_like_by_name('AWESOME')
        }.to change(ResearchActsAsTaggableOn::Tag, :count)

        expect(ResearchActsAsTaggableOn::Tag.last.name).to eq('AWESOME')
      end
    end
    ...
    context 'case sensitive' do
      ...
      it 'should have a named_scope named(something) that matches exactly' do
        uppercase_tag = ResearchActsAsTaggableOn::Tag.create(name: 'Cool')
        @tag.name = 'cool'
        @tag.save!

        expect(ResearchActsAsTaggableOn::Tag.named('cool')).to include(@tag)
        expect(ResearchActsAsTaggableOn::Tag.named('cool')).to_not include(uppercase_tag)
      end
    end
    ...
    it 'should not change encoding' do
      ...
      record = ResearchActsAsTaggableOn::Tag.find_or_create_with_like_by_name(name)
      record.reload
  ...
  describe 'name uniqeness validation' do
    let(:duplicate_tag) { ResearchActsAsTaggableOn::Tag.new(name: 'ror') }

    before { ResearchActsAsTaggableOn::Tag.create(name: 'ror') }
  ...
  describe 'popular tags' do
    before do
      %w(sports rails linux tennis golden_syrup).each_with_index do |t, i|
        tag = ResearchActsAsTaggableOn::Tag.new(name: t)
    ...
    it 'should find the most popular tags' do
      expect(ResearchActsAsTaggableOn::Tag.most_used(3).first.name).to eq("golden_syrup")
      expect(ResearchActsAsTaggableOn::Tag.most_used(3).length).to eq(3)
    end

    it 'should find the least popular tags' do
      expect(ResearchActsAsTaggableOn::Tag.least_used(3).first.name).to eq("sports")
      expect(ResearchActsAsTaggableOn::Tag.least_used(3).length).to eq(3)
    end
  end
