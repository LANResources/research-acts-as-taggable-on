...
describe ResearchActsAsTaggableOn::TagList do
  let(:tag_list) { ResearchActsAsTaggableOn::TagList.new('awesome', 'radical') }
  let(:another_tag_list) { ResearchActsAsTaggableOn::TagList.new('awesome','crazy', 'alien') }
  ...
  describe '#+' do
    ...
    it 'should have class : ResearchActsAsTaggableOn::TagList' do
      new_tag_list = tag_list + another_tag_list
      expect(new_tag_list.class).to eq(ResearchActsAsTaggableOn::TagList)
    end
  end
  ...
  describe '#concat' do
    ...
    context 'without duplicates' do
      let(:arr) { ['crazy', 'alien'] }
      let(:another_tag_list) { ResearchActsAsTaggableOn::TagList.new(*arr) }
  ...
  describe 'cleaning' do
    it 'should parameterize if force_parameterize is set to true' do
      ...
      tag_list = ResearchActsAsTaggableOn::TagList.new('awesome()', 'radical)(cc')

      expect(tag_list.to_s).to eq('awesome, radical-cc')
      ResearchActsAsTaggableOn.force_parameterize = false
    end

    it 'should lowercase if force_lowercase is set to true' do
      ResearchActsAsTaggableOn.force_lowercase = true

      tag_list = ResearchActsAsTaggableOn::TagList.new('aweSomE', 'RaDicaL', 'Entrée')
      expect(tag_list.to_s).to eq('awesome, radical, entrée')

      ResearchActsAsTaggableOn.force_lowercase = false
    end

    it 'should ignore case when removing duplicates if strict_case_match is false' do
      tag_list = ResearchActsAsTaggableOn::TagList.new('Junglist', 'JUNGLIST', 'Junglist', 'Massive', 'MASSIVE', 'MASSIVE')
  ...
  describe 'custom parser' do
    ...
    it 'should use a the default parser if none is set as parameter' do
      allow(ResearchActsAsTaggableOn.default_parser).to receive(:new).and_return(parser)
      ResearchActsAsTaggableOn::TagList.new('cool, wicked', parse: true)
    ...
    it 'should use the custom parser passed as parameter' do
      ...
      ResearchActsAsTaggableOn::TagList.new('cool, wicked', parser: parser_class)
    ...
    it 'should use the parser setted as attribute' do
      ...
      tag_list = ResearchActsAsTaggableOn::TagList.new('example')
