  ...
  module Taggable
    module Core
      def self.included(base)
        base.extend ResearchActsAsTaggableOn::Taggable::Core::ClassMethods
        ...
        base.initialize_research_acts_as_taggable_on_core
      end

      module ClassMethods
        def initialize_research_acts_as_taggable_on_core
          include taggable_mixin
          tag_types.map(&:to_s).each do |tags_type|
            tag_type = tags_type.to_s.singularize
            context_taggings = "#{tag_type}_taggings".to_sym
            context_tags = tags_type.to_sym
            taggings_order = (preserve_tag_order? ? "#{ResearchActsAsTaggableOn::Tagging.table_name}.id" : [])
            ...
            class_eval do
              ...
              has_many context_taggings, -> { includes(:tag).order(taggings_order).where(context: tags_type) },
                       as: :taggable,
                       class_name: 'ResearchActsAsTaggableOn::Tagging',
                       dependent: :destroy,
                       after_add: :dirtify_tag_list,
                       after_remove: :dirtify_tag_list

              has_many context_tags, -> { order(taggings_order) },
                       class_name: 'ResearchActsAsTaggableOn::Tag',
                       through: context_taggings,
                       source: :tag

              attribute "#{tags_type.singularize}_list".to_sym, ResearchActsAsTaggableOn::Taggable::TagListType.new
            end
            ...
            def #{tag_type}_list=(new_tags)
              parsed_new_list = ResearchActsAsTaggableOn.default_parser.new(new_tags).parse

              if self.class.preserve_tag_order? || (parsed_new_list.sort != #{tag_type}_list.sort)
                unless #{tag_type}_list_changed?
                  @attributes["#{tag_type}_list"] = ActiveModel::Attribute.from_user("#{tag_type}_list", #{tag_type}_list, ResearchActsAsTaggableOn::Taggable::TagListType.new)
                end
        ...
        def taggable_on(preserve_tag_order, *tag_types)
          ...
          initialize_research_acts_as_taggable_on_core
        end
        ...
        def tagged_with(tags, options = {})
          tag_list = ResearchActsAsTaggableOn.default_parser.new(tags).parse
          options = options.dup

          return none if tag_list.empty?

          ::ResearchActsAsTaggableOn::Taggable::TaggedWithQuery.build(self, ResearchActsAsTaggableOn::Tag, ResearchActsAsTaggableOn::Tagging,
                                                              tag_list, options)
        end
      ...
      def tag_list_cache_on(context)
        ...
        if instance_variable_get(variable_name)
          ...
          instance_variable_set(variable_name, ResearchActsAsTaggableOn.default_parser.new(cached_tag_list_on(context)).parse)
        else
          instance_variable_set(variable_name, ResearchActsAsTaggableOn::TagList.new(tags_on(context).map(&:name)))
        end
      end
      ...
      def all_tags_list_on(context)
        ...
        instance_variable_set(variable_name, ResearchActsAsTaggableOn::TagList.new(all_tags_on(context).map(&:name)).freeze)
      end

      ##
      # Returns all tags of a given context
      def all_tags_on(context)
        tagging_table_name = ResearchActsAsTaggableOn::Tagging.table_name

        opts = ["#{tagging_table_name}.context = ?", context.to_s]
        scope = base_tags.where(opts)

        if ResearchActsAsTaggableOn::Utils.using_postgresql?
          group_columns = grouped_column_names_for(ResearchActsAsTaggableOn::Tag)
          scope.order(Arel.sql("max(#{tagging_table_name}.created_at)")).group(group_columns)
        else
          scope.group("#{ResearchActsAsTaggableOn::Tag.table_name}.#{ResearchActsAsTaggableOn::Tag.primary_key}")
        end.to_a
      end
      ...
      def tags_on(context)
        scope = base_tags.where([
                                  "#{ResearchActsAsTaggableOn::Tagging.table_name}.context = ? AND #{ResearchActsAsTaggableOn::Tagging.table_name}.tagger_id IS NULL", context.to_s
                                ])
        # when preserving tag order, return tags in created order
        # if we added the order to the association this would always apply
        scope = scope.order("#{ResearchActsAsTaggableOn::Tagging.table_name}.id") if self.class.preserve_tag_order?
        scope
      end
      ...
      def set_tag_list_on(context, new_list)
        ...
        parsed_new_list = ResearchActsAsTaggableOn.default_parser.new(new_list).parse
      ...
      def load_tags(tag_list)
        ResearchActsAsTaggableOn::Tag.find_or_create_all_with_like_by_name(tag_list)
      end
      ...
      ##
      # Override this hook if you wish to subclass {ResearchActsAsTaggableOn::Tag} --
      ...
      #     research_acts_as_taggable_on :markets, :locations
      #
