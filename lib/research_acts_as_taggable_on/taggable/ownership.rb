  ...
  module Taggable
    module Ownership
      def self.included(base)
        base.extend ResearchActsAsTaggableOn::Taggable::Ownership::ClassMethods

        base.class_eval do
          after_save :save_owned_tags
        end

        base.initialize_research_acts_as_taggable_on_ownership
      end

      module ClassMethods
        def research_acts_as_taggable_on(*args)
          initialize_research_acts_as_taggable_on_ownership
          super(*args)
        end

        def initialize_research_acts_as_taggable_on_ownership
      ...
      def owner_tags(owner)
        scope = if owner.nil?
                  ...
                  base_tags.where(
                    ResearchActsAsTaggableOn::Tagging.table_name.to_s => {
                      tagger_id: owner.id,
        ...
        if self.class.preserve_tag_order?
          scope.order("#{ResearchActsAsTaggableOn::Tagging.table_name}.id")
        else
          scope
        end
      end
      ...
      def owner_tag_list_on(owner, context)
        ...
        cache[owner] ||= ResearchActsAsTaggableOn::TagList.new(*owner_tags_on(owner, context).map(&:name))
      end
      ...
      def set_owner_tag_list_on(owner, context, new_list)
        ...
        cache[owner] = ResearchActsAsTaggableOn.default_parser.new(new_list).parse
      end
      ...
      def save_owned_tags
        tagging_contexts.each do |context|
          cached_owned_tag_list_on(context).each do |owner, tag_list|
            ...
            if old_tags.present?
              ResearchActsAsTaggableOn::Tagging.where(taggable_id: id, taggable_type: self.class.base_class.to_s,
