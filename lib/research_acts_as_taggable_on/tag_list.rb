...
module ResearchActsAsTaggableOn
  class TagList < Array
    ...
    def initialize(*args)
      @parser = ResearchActsAsTaggableOn.default_parser
      add(*args)
    end
    ...
    def to_s
      ...
      tags.map do |name|
        d = ResearchActsAsTaggableOn.delimiter
        d = Regexp.new d.join('|') if d.kind_of? Array
        name.index(d) ? "\"#{name}\"" : name
      end.join(ResearchActsAsTaggableOn.glue)
    end
    ...
    def clean!
      ...
      map!(&:to_s)
      map!(&:strip)
      map! { |tag| tag.mb_chars.downcase.to_s } if ResearchActsAsTaggableOn.force_lowercase
      map!(&:parameterize) if ResearchActsAsTaggableOn.force_parameterize

      ResearchActsAsTaggableOn.strict_case_match ? uniq! : uniq!{ |tag| tag.downcase }
      self
    end
